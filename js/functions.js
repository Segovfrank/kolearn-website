...
function initComboBox() {
  ...
  $('.combo-box .box-dropdown li').on('click', function (e) {
    ...
    var itemIconClass = $(this).find('.item-icon i').attr('class');
    var itemSvgIcon = $(this).find('.item-icon')();
  ...
  $('input[name="thumbnails"]').fileuploader({
    ...
    thumbnails: {
      ...
      onItemShow: function onItemShow(item, listEl) {
        ...
        plusInput.insertAfter(item);

        if (item.format == 'image') {
          item.find('.fileuploader-item-icon').hide();
        }
      }
    },
    ...
    upload: {
        ...
        onSuccess: function(data, item) {
            setTimeout(function() {
                item.find('.progress-holder').hide();
                item.renderThumbnail();
            }, 400);
        },
        onError: function(item) {
            item.find('.progress-holder').hide();
            item.find('.fileuploader-item-icon i').text('Failed!');
        },
        onProgress: function(data, item) {
            var progressBar = item.find('.progress-holder');
    ...
    upload: {
      ...
      onSuccess: function onSuccess(result, item) {
        ...
        if (data.isSuccess && data.files[0]) {
          ...
          item.find('.column-title > div:first-child').text(data.files[0].name).attr('title', data.files[0].name);
        } // if warnings
        ...
        if (data.hasWarnings) {
          ...
          item.removeClass('upload-successful').addClass('upload-failed'); // go out from success function by calling onError function
        ...
        item.find('.column-actions').append('<a class="fileuploader-action fileuploader-action-remove fileuploader-action-success" title="Remove"><i></i></a>');
        setTimeout(function () {
          item.find('.progress-bar2').fadeOut(400);
        }, 400);
      },
      onError: function onError(item) {
        var progressBar = item.find('.progress-bar2');

        if (progressBar.length > 0) {
          progressBar.find('span')(0 + "%");
          progressBar.find('.fileuploader-progressbar .bar').width(0 + "%");
          item.find('.progress-bar2').fadeOut(400);
        }

        item.upload.status != 'cancelled' && item.find('.fileuploader-action-retry').length == 0 ? item.find('.column-actions').prepend('<a class="fileuploader-action fileuploader-action-retry" title="Retry"><i></i></a>') : null;
      },
      onProgress: function onProgress(data, item) {
        ...
        if (progressBar.length > 0) {
          progressBar.show();
          progressBar.find('span')(data.percentage + "%");
          progressBar.find('.fileuploader-progressbar .bar').width(data.percentage + "%");
        }
      },
      onComplete: null
...
function initDemo() {
  ...
  $('.accordion-wrapper .trigger a').on("click", function () {
    $(this)('<i class="im im-icon-Coding is-icon-2x"></i> HIDE CODE<i class="im im-icon-Add"></i>');
    $('.trigger.active a')('<i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i>');
  ...
  $('.action-btn').on('click', function () {
    ...
    setTimeout(function () {
      if ($this.text() === 'Register') {
        $this.removeClass('is-loading')('Login');
      } else {
        $this.removeClass('is-loading')('Register');
      }
